...
from tests.test_hosted_managed_operation_executor import ExecutorFixture
    ...
    def do_POST(self) -> None:
        ...
        self._write(
            202,
            {
                "ok": True,
                "receipt": receipt,
                "summary": {"request_id": type(self).request_payload["request_id"]},
            },
        )
...
class HostedManagedOperationCanaryTests(unittest.TestCase):
    ...
    def test_canary_rejects_irreversible_operation_without_explicit_dry_run(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            fixture = ExecutorFixture(Path(temp_dir))
        ...
        self.assertIn("canary operation must be read-only", " ".join(report["diagnostics"]))
