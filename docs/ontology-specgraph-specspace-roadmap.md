...
4. **Hosted/queue-backed managed execution.** Implemented for the current
   ...
   evidence.
   Before the next expansion, keep the Internet access-control boundary closed.
   SpecSpace now has a backend-enforced single-operator profile deployed in
   production.
   ...
   An experimental macOS Keychain credential source is implemented for this
